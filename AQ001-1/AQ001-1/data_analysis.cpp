...
void Data_Analysis::UpdateData()
{
    ...
    switch (ui->ch1_comboBox->currentIndex()) {
    case 0x00:
        m_queue_y1->EnQueue(position_x);
        y1_max = y1_max>fabs(position_x) ? y1_max : fabs(position_x);//get max y value.
        break;
    case 0x01:
        m_queue_y1->EnQueue(d_position_x);
        y1_max = y1_max>fabs(d_position_x) ? y1_max : fabs(d_position_x);//get max y value.
        break;
    case 0x02:
        m_queue_y1->EnQueue(theta);
        y1_max = y1_max>fabs(theta) ? y1_max : fabs(theta);//get max y value.
        break;
    case 0x03:
        m_queue_y1->EnQueue(d_theta);
        y1_max = y1_max>fabs(d_theta) ? y1_max : fabs(d_theta);//get max y value.
        break;
    case 0x04:
        m_queue_y1->EnQueue(roll);
        y1_max = y1_max>fabs(roll) ? y1_max : fabs(roll);//get max y value.
        break;
    case 0x05:
        m_queue_y1->EnQueue(pitch);
        y1_max = y1_max>fabs(pitch) ? y1_max : fabs(pitch);//get max y value.
        break;
    case 0x06:
        m_queue_y1->EnQueue(yaw);
        y1_max = y1_max>fabs(yaw) ? y1_max : fabs(yaw);//get max y value.
        break;
    default:
        break;
    }

    switch (ui->ch2_comboBox->currentIndex()) {
    case 0x00:
        m_queue_y2->EnQueue(position_x);
        y2_max = y2_max>fabs(position_x) ? y2_max : fabs(position_x);//get max y value.
        break;
    case 0x01:
        m_queue_y2->EnQueue(d_position_x);
        y2_max = y2_max>fabs(d_position_x) ? y2_max : fabs(d_position_x);//get max y value.
        break;
    case 0x02:
        m_queue_y2->EnQueue(theta);
        y2_max = y2_max>fabs(theta) ? y2_max : fabs(theta);//get max y value.
        break;
    case 0x03:
        m_queue_y2->EnQueue(d_theta);
        y2_max = y2_max>fabs(d_theta) ? y2_max : fabs(d_theta);//get max y value.
        break;
    case 0x04:
        m_queue_y2->EnQueue(roll);
        y2_max = y2_max>fabs(roll) ? y2_max : fabs(roll);//get max y value.
        break;
    case 0x05:
        m_queue_y2->EnQueue(pitch);
        y2_max = y2_max>fabs(pitch) ? y2_max : fabs(pitch);//get max y value.
        break;
    case 0x06:
        m_queue_y2->EnQueue(yaw);
        y2_max = y2_max>fabs(yaw) ? y2_max : fabs(yaw);//get max y value.
        break;
    default:
        break;
    }
    ...
    yList_1 = m_queue_y1->QueueTraverse(get_cnt);
    yList_2 = m_queue_y2->QueueTraverse(get_cnt);
    ...
    ui(new Ui::Data_Analysis)
{
    ...
    m_queue_y1 = new Circle_Queue<float>(20000);
    m_queue_y2 = new Circle_Queue<float>(20000);
...
void Data_Analysis::on_plot_reset_released()
{
    ...
    m_queue_y1->ClearQueue();
    m_queue_y2->ClearQueue();
