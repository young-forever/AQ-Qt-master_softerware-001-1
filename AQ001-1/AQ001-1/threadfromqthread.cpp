...
{
    static int addr;
    char* data_temp;//暂时只用到一位值

    do {
        int rdlen;

//        cout<<"before"<<endl;
        rdlen = my_serial_->my_serial_read(data_temp,1);//read my serial.
        ...
        while(buff_arry.size() >= 8)//juge whether the
        {
            ...
            {
                if( buff_arry.size() < 35)
                {
                    break;
                }

                if(buff_arry.data()[34] == 0x0D)
                {
                    unsigned char re_command = buff_arry[1];//功能码
                    ...
                    QByteArray long_re_data = buff_arry.left(32);//指令数据位, 6位Byte

                    my_serial_->long_command_execute((unsigned char*)long_re_data.data());//执行指令解析操作

                    buff_arry.remove(0,32);//移除执行过的数据帧
                }
                else
                {
                    buff_arry.remove(0,1);//移除执行过的数据帧
